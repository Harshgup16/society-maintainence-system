...
const OWNER_FALLBACK_EMAIL = 'hkgupta160420@gmail.com'; // Verified Resend owner email for test mode
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; background-color: #f0ece4; padding: 24px; border-radius: 12px; color: #1a1a1a;">
      <div style="background-color: #ffffff; padding: 24px; border-radius: 8px; border: 1px solid #d1cdc5;">
        ...
        <p>Your account has been successfully created! You can now log in to raise maintenance complaints, upload photos, track issue resolution in real-time, and stay updated with society notices.</p>
        ...
        <p style="margin-bottom: 0; margin-top: 24px; color: #8a8a8a; font-size: 12px;">Society Maintenance Tracker • Registered Email: ${residentEmail}</p>
      </div>
    </div>
  `;

  try {
    // 1. Try sending directly to resident
    ...
    if (data.error && data.error.statusCode === 403) {
      console.warn('[Resend Free Tier Restricted Target] Retrying send to verified account owner:', OWNER_FALLBACK_EMAIL);
      const fallbackData = await resend.emails.send({
        from: FROM_EMAIL,
        to: [OWNER_FALLBACK_EMAIL],
        subject: `[FORWARDED TO ADMIN] ${subject} (Recipient: ${residentEmail})`,
        html: `<p style="background:#fff3cd;padding:10px;border-radius:6px;font-size:12px;"><strong>Note:</strong> In Resend Free Testing Mode, emails to unverified recipients are forwarded to project owner (${OWNER_FALLBACK_EMAIL}). Target Resident: ${residentEmail}</p>${html}`,
      });
...
export async function sendImportantNoticeEmail({
  ...
  try {
    const data = await resend.emails.send({
      from: FROM_EMAIL,
      to: [OWNER_FALLBACK_EMAIL], // Safe send in test mode
      subject,
      html,
    });
